//! `pipa concepts` — explain the access / zone / step-up model offline.
...
pub async fn run(json: bool) -> Result<()> {
    if json {
        let v = serde_json::json!({
            ...
            "zone": {
                ...
                "note": "only enforced when the server has the `zone` feature (see `pipa server`)"
            },
    ...
    println!("pipa access / zone model\n");
    ...
    println!("  (only enforced when the server has the `zone` feature — see `pipa server`)\n");
