...
pub async fn run(args: LoginArgs) -> Result<()> {
    ...
    let setup_code: String = Input::with_theme(&theme)
        .with_prompt(format!(
            "setup code (printed by `gapes-server` on first boot; leave blank if you have an active session on this host)"
        ))
        .allow_empty(true)
        .interact_text()?;
            ...
            if e.status() == Some(401) {
                bail!(
                    "server requires an existing session to register another device.\n\
                     mint a fresh setup code on the host with `gapes-server setup`, then re-run `gapes login`."
                );
            }
