//! Shared test harness for the `gapes-server` integration tests.
//!
...
use gapes_adapters::Config;
use gapes_server::{AppState, build_app_state_for_test, build_router};
...
pub fn mint_access(state: &AppState, sub: &str, scope: &str, ttl_sec: i64) -> String {
    let (tok, _claims) =
        gapes_server::auth::tokens::mint_access_token(&state.hmac_key, sub, scope, ttl_sec)
            .expect("mint access");
    tok
}
