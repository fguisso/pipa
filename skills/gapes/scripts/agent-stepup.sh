#!/bin/sh
# agent-stepup.sh — run a gapes command that requires a step-up confirmation
...
# e.g. `gapes get <uuid> --json`. The raw result is also written to the temp
# file printed on stderr.
set -eu

[ $# -ge 1 ] || { echo "usage: agent-stepup.sh <gapes-args...>" >&2; exit 2; }
command -v gapes >/dev/null 2>&1 || { echo "gapes is not installed (run the installer first)" >&2; exit 1; }

OUT="$(mktemp)"
nohup gapes "$@" --json >"$OUT" 2>&1 &
