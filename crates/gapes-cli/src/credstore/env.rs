...
const ENV_VAR: &str = "GAPES_REFRESH_TOKEN";

/// Read-only credstore backed by the `GAPES_REFRESH_TOKEN` env var. Used by
...
impl CredStore for EnvStore {
    ...
    fn display_name(&self) -> &'static str {
        "GAPES_REFRESH_TOKEN env var"
    }
    fn security_label(&self) -> &'static str {
        "●●○○○ ci-only"
    }
    fn store(&self, _server: &str, _refresh: &str) -> Result<()> {
        bail!("GAPES_REFRESH_TOKEN is read-only; unset it to use a writable credstore")
    }
    ...
    fn delete(&self, _server: &str) -> Result<()> {
        bail!("GAPES_REFRESH_TOKEN is read-only; unset it manually")
    }
}
