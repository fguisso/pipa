...
/// Errors with a friendly "run `pipa login`" if no token exists.
...
///
/// If the active tier is read-only (e.g. `PIPA_REFRESH_TOKEN`), persistence
    ...
    if !features.iter().any(|f| f == feature) {
        anyhow::bail!(
            "this server does not enforce `{feature}` — `{flag}` would be stored but ignored. \
             Re-run with --force to send it anyway, or check `pipa server`."
        );
    }
    Ok(())
}
