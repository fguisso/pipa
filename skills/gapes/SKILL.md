...
name: gapes
description: >-
  Install and drive the `gapes` CLI to deploy/manage static pages on a gapes
  ...
  site, change a page's access/zone, list/inspect pages, or set up the gapes CLI.
---

# gapes — client skill (agent-agnostic)

This skill drives the `gapes` CLI. It works from any agent (Codex, Claude Code,
...
Run `gapes concepts` (or `gapes concepts --json`) any time you need the model
spelled out without a network call.

## 0. Ensure the CLI is installed

```sh
gapes --version || curl -fsSL https://guisso.dev/gapes/install.sh | sh
```
...
2. If there's no server to reuse, **ASK the human: "What is the upstream gapes
...
3. Start the login and hand the URL to the human:
   ...
   in as the gapes admin/owner).
4. Wait for them, then confirm:
   ```sh
   gapes whoami --json    # expect "logged_in":true
   ```

Credentials are stored locally (best tier available, falling back to
`~/.config/gapes/auth.toml`, chmod 600). The chosen server is remembered, so
later commands don't need `--server`.

## 2. Know what the server supports

```sh
gapes server --json   # {"server":"…","features":["zone", …]}
```
...
```sh
gapes deploy ./dist --json                                  # access=password, default zone
gapes deploy ./dist --access noauth --json                  # open page
gapes deploy ./dist --zone public --json                    # internet (needs server `zone` feature)
```
...
```sh
gapes share <uuid> --zone private --json
```
...
- `gapes activity` does not yet have a CLI read endpoint on older servers; the
  audit log lives server-side.
- Deleting (`gapes rm <uuid>`) also needs step-up → use `agent-stepup.sh rm <uuid>`.
