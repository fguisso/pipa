#!/bin/sh
# agent-login.sh — start a gapes device-flow login and print ONLY the verify
...
# afterwards with `gapes whoami --json` (look for "logged_in":true).
#
...
nohup gapes login --server "$SERVER" --label "$LABEL" --json >"$OUT" 2>&1 &
